Public Class Damped
    ...
    Const MyMethodName As String = "Damped Exponential Smoothing"
...
#Region "Properties"
    ...
    Public Property phi As Double
        Get
            Return _Parameters("phi")
        End Get
        Set(value As Double)
            _Parameters("phi") = value
        End Set
    End Property
...
#End Region
    ...
    ' ''' <summary>
    ' ''' Calculates the Forecast Model of the Damped Exponential Smoothing Method
    ' ''' </summary>
    ...
    ' ''' <param name="phi">The value of smoothing parameter for the trend-modification, if the value '-1' is assigned to phi, the method will optimize the parameter</param>
    ...
    ' ''' <remarks></remarks>
    'Public Sub Calculate(ByVal Input As Timeseries, ByVal Horizon As Integer, Optional ByVal S0 As Double = Double.NaN, _
    '                  Optional ByVal T0 As Double = Double.NaN, Optional a As Double = -1, Optional b As Double = -1, _
    '                  Optional phi As Double = -1, Optional ByVal ErrorMethod As ErrorMethodForOptimization = Nothing)
    '    Dim Paras = Optimization(ErrorMethod, Input, S0, T0, a, b, phi)
    ...
    '    _Parameters.Add("phi", Paras.Item5)
    'End Sub
...
#Region "Functions"
    ...
    Public Overrides Sub Calculate()
        Try
            ...
            Dim SesTs = CalculateForecastTimeseries(_Horizon, _Parameters("S0"), _Parameters("T0"), _Parameters("a"), _Parameters("b"), _Parameters("phi"))
    ...
    Public Overrides Sub OptimizeParameters()
        ...
        Dim tempphi = IIf(_Parameters.TryGetValue("phi", Value), Value, -1)

        Dim Paras = Optimization(_ErrorMethod, tempS0, tempT0, tempa, tempb, tempphi)
        _Parameters("S0") = Paras.Item1
        _Parameters("T0") = Paras.Item2
        _Parameters("a") = Paras.Item3
        _Parameters("b") = Paras.Item4
        _Parameters("phi") = Paras.Item5
    End Sub
    Public Overrides Function MyType() As ForecastFactory.ForecastModel
        Return ForecastFactory.ForecastModel.Damped
    End Function
#End Region
